		...
		void MyComboBox_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (_source != null)
			{
				try
				{
					...
					if (!Planner.comPort.setParam(ParamName, (float)(Int32)Enum.Parse(_source, this.Text)))
					{
					...
					{
						if (
							!Planner.comPort.setParam(paramname2,
			...
			else if (_source2 != null)
			{
				try
				{
					...
					if (!Planner.comPort.setParam(ParamName, (float)(int)((MyComboBox)sender).SelectedValue))
					{
