...
namespace Diva.Controls.Pages
{
    class PageManager : IDisposable
    {
        ...
        public PageManager(Control Parent)
        {
        ...
        void CreateControl(Screen current)
        {
            ...
            if (MainControl.InvokeRequired)
            {
                MainControl.Invoke((MethodInvoker)delegate
                {
                    try
                    {
                        current.Control = (UserControl)Activator.CreateInstance(type);
                    }
                    catch (Exception ex)
                    {
                        throw new Exception(
        ...
        public class Screen
        {
            ...
            public Screen(string Name, UserControl Control, bool Persistent = false)
            {
                this.Name = Name;
